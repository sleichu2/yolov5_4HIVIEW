...
{
	HI_U32 i = 0, j = 0;
	HI_U32 u32Offset = 0; //地址偏移量
	HI_U32 u32Num = 0;
	for(i = 0; i < pstNnieParam->pstModel->u32NetSegNum; i++) //网络分段数目
	{
        /*fill forwardCtrl info*/
//        printf("pstNnieParam->pstModel->astSeg[i].enNetType = %d\n", pstNnieParam->pstModel->astSeg[i].enNetType);
		if(SVP_NNIE_NET_TYPE_ROI == pstNnieParam->pstModel->astSeg[i].enNetType) // 输入类型为带有ROI的网络，比如FasterRCNN的后半部分
		{
            ...
            SVP_NNIE_NET_TYPE_RECURRENT== pstNnieParam->pstModel->astSeg[i].enNetType) //输入类型为CNN或者RECURRENT
		{
			pstNnieParam->astForwardCtrl[i].enNnieId = pstNnieCfg->aenNnieCoreId[i]; //使用的nnie的ID
			pstNnieParam->astForwardCtrl[i].u32SrcNum = pstNnieParam->pstModel->astSeg[i].u16SrcNum; //第i段网络的输入数目
			pstNnieParam->astForwardCtrl[i].u32DstNum = pstNnieParam->pstModel->astSeg[i].u16DstNum; //第i段网络的输入数目
        ...
        /*fill src blob info*/
		for(j = 0; j < pstNnieParam->pstModel->astSeg[i].u16SrcNum; j++) // 遍历当前段的每一个输入
	    {
            /*Recurrent blob*/
...
HI_S32 SAMPLE_COMM_SVP_NNIE_LoadModel_FromMem(unsigned char *pszModel, long slFileSize,
                                              SAMPLE_SVP_NNIE_MODEL_S *pstNnieModel)
{
    HI_S32 s32Ret = HI_INVALID_VALUE;
    HI_U64 u64PhyAddr = 0;
    ...
    pstNnieModel->stModelBuf.u64PhyAddr = u64PhyAddr;
    pstNnieModel->stModelBuf.u64VirAddr = (HI_U64)(HI_UL)pu8VirAddr;
    memcpy(pu8VirAddr, pszModel, slFileSize);
